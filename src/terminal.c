...
void display_messages(MessageBuffer *mb, char *clientName)
{
    ...
    int visible = g_term_height - 2;
    if(visible < 1)
        visible = 1;

    int start_index = mb->msg_count - visible + g_scroll_offset;
    if(start_index < 0)
        start_index = 0;

    int end_index = mb->msg_count + g_scroll_offset;
    if(end_index > mb->msg_count) 
        end_index = mb->msg_count;
    ...
    {
        if(mb->message_types[i] == MSG_SELF)
        {
            printf(ANSI_BG_CYAN "You:" ANSI_RESET " %s\n", mb->messages[i]);
        }
        else if(mb->message_types[i] == MSG_PEER)
        {
            printf(ANSI_BG_GREEN "%s:" ANSI_RESET " %s\n", clientName != NULL ? clientName : "user", mb->messages[i]);
        }
        else
        {
            printf("%s\n", mb->messages[i]);
...
{
    char safe[MAX_INPUT];
    sanitize_for_display(text, safe, sizeof(safe));

    pthread_mutex_lock(&g_msg_buffer.lock);

    if(g_msg_buffer.msg_count >= MAX_MESSAGES)
    {
        memmove(g_msg_buffer.messages[0], g_msg_buffer.messages[1], sizeof(g_msg_buffer.messages[0]) * (MAX_MESSAGES - 1));

        memmove(&g_msg_buffer.message_types[0], &g_msg_buffer.message_types[1], sizeof(int) * (MAX_MESSAGES - 1));

        g_msg_buffer.msg_count = MAX_MESSAGES - 1;
    }

    memcpy(g_msg_buffer.messages[g_msg_buffer.msg_count], safe, MAX_INPUT);
